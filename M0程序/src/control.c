...
{
	char sendbuf[24] = {0};
	SyncData(sto_no);	//将采集到的数据同步到环境信息结构体里面
	if(To1SFlag == TRUE)	//1s进行一次发送
	{
...
{	
	...
	if(To100msFlag == TRUE)
	{
		...
		EnvMenuBuf[0][11] = temperature[0] / 10 + 0x30;//加30是转换成ASII码
...
void Control(uint8_t sto_no)
{
	...
	if(RfidGetFinishFlag == TRUE)   //在Sacnrifd()---->ScanAllSensor()函数中变化的，即有刷卡
	{	
			//UART0_PutChar('E');
